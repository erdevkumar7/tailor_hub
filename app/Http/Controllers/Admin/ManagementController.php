    ...
    //   print_r($city);die;
      ...
      if($vendor_id==0){
        // echo "test";die;
            if ($request->hasFile('profile_image')) {
              $image = $request->file('profile_image');
              $imageName = "tai".time().'.'.$image->getClientOriginalExtension();
              $image->move(public_path('/upload/vendor'), $imageName);
          }
        //   $user = Auth::guard("customer")->user();
          $vendor = new Vendor;
        ...
        if ($request->hasFile('profile_image')) {
            ...
            $imageName = $vendor->profile_img;  // Retain existing image if no new image is uploaded
        }
       ...
       DB::table('vendors')
          ...
          Session::flash('message', 'Vendor Updated Sucessfully!');
          return redirect()->to('/admin/tailor-list');
          }elseif($vendor->vendor_type == 2){
            Session::flash('message', 'Vendor Updated Sucessfully!');
            return redirect()->to('/admin/fabric-seller-list');
          }elseif($vendor->vendor_type == 3){
            Session::flash('message', 'Vendor Updated Sucessfully!');
            return redirect()->to('/admin/tailor-seller-list');
          }
      }
  }
...
public function vendorFilter(Request $request)
{
    ...
    // print_r($request->all());die;
    $user_list = Vendor::query();

    if ($request->customer_type) {
        $user_list->where('vip_type', $request->customer_type);
        // $user_list->where('vip_type', 0);
    }

    if ($request->account_status) {
        $user_list->where('vendor_status', $request->account_status);
    }
    ...
    return view('admin.vendor.tailor_filter', compact('user_list'));
}


public function vendorFilter_old(Request $request) {

    // echo "test";die;
    // print_r($request->all());die;

    $user_list = Vendor::where('vip_type', $request->customer_type)->orWhere('vendor_status', $request->account_status)->get();
...
public function tailor_view($id = ""){
    //  print_r($id);die;
    $user_detail = Vendor::where('vendor_id',$id)->first();
    // echo "<pre>";print_r($user_detail);die;
    ...
    $state = DB::table('master_state')->select('state_id','state_name')->where('state_id',$user_detail->state_id)->first();
    $city = DB::table('master_city')->select('city_id','city_name')->where('city_id',$user_detail->city_id)->first();
    return view('admin.vendor.tailor_view',compact('user_detail','countries','state','city'));

}

public function tailor_edit($id = ""){
    //  print_r($id);die;
    $user_detail = Vendor::where('vendor_id',$id)->first();
    // echo "<pre>";print_r($user_detail);die;
    $countries = DB::table('master_country')->select('country_id','country_name')->where('country_id',$user_detail->country_id)->get();
    ...
    $city = DB::table('master_city')->select('city_id','city_name')->where('city_id',$user_detail->city_id)->get();
    // print_r($countries);die;
    return view('admin.vendor.tailor_edit',compact('user_detail','countries','state','city'));
...
public function fabric_seller_form(){
    // echo "test";die;
    $country = DB::table('master_country')->where('country_id','=','161')->select('country_id','country_name')->get();
    return view('admin.vendor.fabric_seller_form',compact('country'));
  }

  public function fabricSellerAction(Request $request)
  {
    // echo "test";die;
    //  echo "<pre>";  print_r($request->all());die;
      // dd($request);
    ...
    //   print_r($city);die;
      ...
      if($vendor_id==0){
        ...
        //   $vendor->gender = $request->gender;
          ...
          Session::flash('message', 'Fabric Seller Inserted Sucessfully!');
  ...
  public function tailor_seller_list(){
    ...
    public function tailor_seller_form(){
        // echo "test";die;
        $country = DB::table('master_country')->where('country_id','=','161')->select('country_id','country_name')->get();
        return view('admin.vendor.tailor_seller_form',compact('country'));
      }


      public function tailorSellerAction(Request $request)
      {
        // echo "test";die;
        //  echo "<pre>";  print_r($request->all());die;
          // dd($request);
        ...
        //   print_r($city);die;
          ...
          if($vendor_id==0){
            ...
            //   $vendor->gender = $request->gender;
              ...
              Session::flash('message', 'Tailor & Fabric Seller Inserted Sucessfully!');
