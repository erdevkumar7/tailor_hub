...
class HomeController extends Controller
{
	...
	// 		"password" => "required"
	// 	]);
	// 	// Get the current session ID before login
	...
	// 		// Find the cart associated with the current session ID
	// 		$sessionCart = Cart::where('session_id', $previousSessionId)->first();

	// 		if ($sessionCart) {
	// 			// Update the cart to associate it with the logged-in customer
	// 			$sessionCart->update([
	// 				'customer_id' => $customerId,
	// 				'session_id' => null // Nullify the session ID since it's now tied to the customer
	// 			]);
	// 		}
	...
	public function catalogueDetail($id)
	{
		...
		// echo "<pre>";print_r($data['related_image']);die;
		$data['vendor'] = Vendor::where('vendor_id', $data['catalogue']->vendor_id)->first();

		// $data['relatedcatalogue'] = Catalogue::where('category_id',$data['catalogue']->category_id)->where('id','!=',$id)->get();
		$data['relatedcatalogue'] = DB::table('catalogue')
			...
			->get();
		// echo "<pre>";	print_r($data['relatedcatalogue']);die;
