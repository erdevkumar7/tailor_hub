...
public function tailor_design_detail(Request $request){
    ...
    try {
        ...
        foreach ($catalogue_detail as $value) {
            // Get catalogue images for each catalogue
            $total = DB::table('reviews')->where('product_id',$catalogue_detail[0]->id)->where('type',"3")->sum('rate');
            $quantity = DB::table('reviews')->where('product_id',$catalogue_detail[0]->id)->where('type',"3")->count();
        ...
        $response['status'] = 1;
        $response['message'] = 'Catalogue Listing.';
        $response['data'] = ['catalogue_detail' => $catalogue_list,'suggested_fabric' => $catalogue_list  ];
    } catch (\Exception $e) {
        // Catch any errors and prepare the error response
        $response['message'] = 'An unexpected error occurred: ' . $e->getMessage();
    }
