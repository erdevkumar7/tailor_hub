    ...
    try {
        ...
        if ($user) {
            if ($user->user_status == 1) {
                ...
                // Mail::send('emails.email_otp', ['otp' => $otpCode], function ($message)  {
                //     $message->to($emailId)
                //         ->subject('Verify Your Email');
                // });
...
public function social_login(Request $request)
{
    ...
    try {
        ...
        if (!$user) {
            ...
            //     'email_id' => $validatedData['sender_id'],
            //     'social_id' => $validatedData['social_id'],
            //     'device_id' => $validatedData['deviceid'],
            //     'device_type' => $validatedData['devicetype'],
            //     'is_social' => 1, // Mark as a social login user
            //     'user_status' => 1, // Optionally activate user
            // ]);
        }
